...
public class AirLineShow extends JFrame {
	...
	class RecieveAndAnalyse extends SwingWorker<String, DataBean> {
		 ...
		 * 传给process()方法，用于界面的显示 publish()方法是传递中间进度数据到事件分配线程，在doInBackground()中调用该方法
		 * (non-Javadoc)
		 * 
		...
		/*
		 * 覆盖这一方法来处理事件分配进程中的传过来的中间进度数据 将传过来的数据显示到界面上，用来更新界面(non-Javadoc)
		 * 
